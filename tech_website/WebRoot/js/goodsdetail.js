...
$(function() {
	...
	var BindEvents = function() {
						...
						if (_RID != null && parseInt(_RID) > 0) {
							...
							var ShareCallFun = function(json) {
									if (json.code == 1 && json.data.urls[0].result) {
										ShareUrl = json.data.urls[0].url_short.replace("t.cn", $www);
										loadBDShareJS()
									}
								};
							$.getJSON("https://api.weibo.com/2/short_url/shorten.json?source=1681459862&url_long=" + refUrl() + "&callback=?", ShareCallFun)
						} else {
								...
								if (data != "") {
									...
									if (len > 0) {
										...
										for (var i = 0; i < len; i++) {
											...
											_html += "<dt>" + listItems[i].buyDate + "</dt>";
											_html += "<dd>" + listItems[i].randomNumbers.replace(
											new RegExp(",", "gm"), "</dd><dd>") + "</dd>";
											_html += "</dl>"
										}
										_RecordObj.html(_html);
										var _showall = _RecordObj.next("a");
										if (_k > _GetCount && _showall.is(":hidden")) {
											_showall.show().attr("href", "/user/UserBuyDetail-" + _CodeID + ".html")
										} else {
											var _TotalHeight = 0;
											_RecordObj.children("dl").each(
										...
										_RecordObj.html('<div class="null-mytips-wrapper clearfix"><div class="gth-icon transparent-png"></div><span class="my-text">您还没有参与本期商品哦！</span></div>')
									}
								} else {
									_RecordObj.html('<div class="null-mytips-wrapper clearfix"><div class="gth-icon transparent-png"></div><span class="my-text">您还没有参与本期商品哦！</span></div>')
								}
				...
				function(e) {
					...
					if (userBuyRNOCache["userBuyRnoArr" + _BuyID] != null) {
						...
						if (userBuyRNOCache["userBuyRnoStr" + _BuyID] != null) {
							...
							$.ajax({
								url: "/products/getUserBuyCodeByBuyid.action",
								data: "id=" + _BuyID,
								success: function(data) {
									if (data != "false") {
										var listItems = data;
										...
										success: function(data) {
											if (data.length > 0) {
												...
												for (var i = 0; i < listItems.length; i++) {
													...
													_html += "<li>";
													_html += '<span class="time">' + item.buyDate + "</span>";
													_html += '<span class="name"><span class="w"><a href="/u/' + item.userId + '.html" target="_blank" title="' + item.userName + '"><i class="head-s-img"><img src="' + item.userFace + '" width="22" height="22"></i>' + item.userName + "</a></span></span>";
													_html += '<span class="people">' + item.buyCount + "<a name='showCode' buyid='" + item.buyId + "' uid='" + item.userId + "' buynum='" + item.buyCount + '\' href="javascript:;">查看夺宝码</a></span>';
													_html += '<span class="ip">' + item.ip_location + " IP:" + item.ip_address + "</span>";
													if (item.buySource == "1") {
														_html += '<span class="form"><a href="/#" target="_blank">手机触屏版<i class="f-icon mobile"></i></a></span>'
													} else {
														if (item.buySource == "2") {
															_html += '<span class="form"><a href="/#" target="_blank">Android客户端<i class="f-icon andriod"></i></a></span>'
														} else {
															if (item.buySource == "3") {
																_html += '<span class="form"><a href="/#" target="_blank">iOS客户端<i class="f-icon ios"></i></a></span>'
															} else {
																if (item.buySource == "4") {
																	_html += '<span class="form"><a href="/#" target="_blank">微信公众平台<i class="f-icon wx"></i></a></span>'
																} else {
												...
												_BuyListCache["page" + pageNo] = _RecordList.find("ul").html();
												_isLoaded = false
											} else {
												if (curMenuIndex > 0) {
			...
			function loading() {
				$("#div_allrecord").children("div.record").find("ul").html('<div class="ng-b-wait"></div>')
			}

			function loading2() {
				$("#div_postlist").find("div.ng-share-wrapper").html('<div class="ng-b-wait"></div>')
			}
					...
					var getEventObj = function(html) {
							...
							function(idx) {
								...
								function(e) {
									...
									_picHover.unbind("click").bind("click", function(
									e) {
										stopBubble(e);
										_index = idx;
										$("body").find("div.ng-show-bigpic").remove();
										var _bigPic = $('<div class="ng-show-bigpic" style="display:none;"><img src="' + $img + _This.attr("pic") + '"></div>');
										$("body").append(
										_bigPic);
										_bigPic.hover(
							...
							function pageselectCallback2(pageNo) {
								if (_PostListCache["page" + pageNo] != null) {
									...
									$.ajax({
										...
										data: "id=" + _GoodsID + "&pageNo=" + pageNo,
										beforeSend: loading2,
										success: function(Z) {
											if (Z.length > 0) {
												_PostObj.find("div.ng-share-wrapper").before('<div class="total">本商品已有<span class="gray6">' + resultCount + "</span>位幸运用户晒单 </div>");
												var listItems = Z;
												var _html = "";
												for (var i = 0; i < listItems.length; i++) {
													_html += '<div class="ng-share-detail clearfix">';
													_html += '<div class="ng-share-pic">';
													_html += '<a href="/u/' + listItems[i].userId + '.html" type="showCard" uweb="' + listItems[i].userId + '"  target="_blank">';
													_html += '<img src="' + listItems[i].userFace + '" alt="" width="80" height="80" /></a>';
													_html += "</div>";
													_html += '<div class="ng-share-con">';
													_html += '<div class="name-line">';
													_html += '<a href="/u/' + listItems[i].userId + '.html" target="_blank" rel="nofollow" class="u-name" >' + listItems[i].userName + "</a>";
													_html += '<span class="u-time">' + listItems[i].shareDate + "</span>";
													_html += "</div>";
													_html += '<div class="u-data">';
													_html += '<span class="u-num">第<span>' + listItems[i].productPeriod + "</span>云晒单</span>";
													_html += '<a href="/shareShow/' + listItems[i].uid + '.html" class="u-show" target="_blank">' + listItems[i].shareTitle + "</a>";
													_html += "</div>";
													_html += '<div class="share-info">';
													_html += '<p><a href="/shareShow/' + listItems[i].uid + '.html"  target="_blank">' + listItems[i].shareContent.substring(0, 150) + (listItems[i].shareContent.length > 150 ? "..." : "") + "</a></p>";
													_html += "</div>";
													...
													for (var j = 0; j < _picLen; j++) {
														_html += "<li pic=/UserPost/Big/" + goodsPic[j].images + ">";
														_html += "<span>";
														_html += '<img width="71px" height="71px" src="/Images/loading71.gif" src2="' + $img + "/UserPost/Small/" + goodsPic[j].images + '" /></span>';
														_html += '<div class="pic-hover transparent-png" style="display:none;"></div>';
														_html += "</li>"
													}
													_html += "</ul>";
													_html += '<div class="talk-wrap">';
													_html += '<a href="javascript:;" num=\'' + listItems[i].upCount + "' postid='" + listItems[i].uid + '\' class="xianmu">';
													_html += '<i class="transparent-png"></i>';
													_html += "羡慕(<em>" + listItems[i].upCount + "</em>)";
													_html += '<img class="transparent-png" src="/Images/xin.png">';
													_html += "</a>";
													_html += '<a rel="nofollow" href="/shareShow/' + listItems[i].uid + '.html" target="_blank" class="pinglun">';
													_html += '<i class="transparent-png"></i>';
													_html += "评论(<em>" + listItems[i].replyCount + "</em>)";
													_html += "</a>";
													_html += "</div>";
													_html += "</div>";
													_html += "</div>";
												...
												loadImgFun();
												_PostListCache["page" + pageNo] = _PostObj.find("div.ng-share-wrapper").html();
								...
								$.ajax({
									...
									success: function(data) {
										if (data == "false") {
											...
											if (json == null || json == "") {
												var val = "[{'pId':" + codeid + ",'num':" + _Num + "}]";
												SetCookieByExpires("products", val, 1)
											} else {
												var flag = false;
												json = eval("(" + json + ")");
												$.each(
												json, function(n, value) {
													if (codeid == value.pId) {
			...
			var bindLotteryTimeFun = function() {
					$.getScript(skinDomain + "/js/goodsdetaillottery.js?date=141130", function() {
						startTimeOut();
						showMenuFun(1)
					})
				};
			...
			if ($("#li_more").length > 0) {
				var LoadPaneJs = function() {
						$.getScript(
						skinDomain + "/js/jquery.jscrollpane.js?date=20150513", function() {
							$("#li_more").bind("click", function(e) {
								...
								var showMorePeriodFun = function() {
										...
										var GetListDataFun = function() {
												var getParaFun = function() {
														var _Paras = "";
														_Paras += "id=" + _SendData.goodsID;
														_Paras += "&codeID=" + _SendData.codeID;
														_Paras += "&pageNo=" + _SendData.FIdx;
														_Paras += "&pageSize=" + _SendData.EIdx;
														...
														if (result.length > 0) {
															...
															if (CBLFun.TotalCount > 0) {
																...
																for (var i = 0; i < _listItems.length; i++) {
																	...
																	if (item.status == 0) {
																		if (item.spellbuyProductId == _CodeID) {
																			_html += '<li class="current"><a href="/products/' + item.productId + '.html">第' + item.productPeriod + '期<b>进行中<span class="dotting"></span></b></a></li>'
																		} else {
																			_html += '<li><a class="orange" href="/products/' + item.productId + '.html">第' + item.productPeriod + '期<b>进行中<span class="dotting"></span></b></a></li>'
																		}
																	} else {
																		if (item.spellbuyProductId == _CodeID) {
																			_html += '<li class="current"><a href="/products/' + item.spellbuyProductId + '.html">第' + item.productPeriod + "期</a></li>"
																		} else {
																			_html += '<li><a href="/products/' + item.spellbuyProductId + '.html">第' + item.productPeriod + "期</a></li>"
																		}
																	}
																}
																...
																if (CBLFun.TotalCount > _showcount) {
																	...
																	_api.unbind("mousewheel").bind("mousewheel", function(
																	e, delta) {
																		var dir = delta > 0 ? "Up" : "Down";
																		var scrollTop = _api.scrollTop();
																		var scrollHegiht = _api[0].scrollHeight;
								...
								var ReadyFun = function(
									showObj) {
										...
										_DivMainObj.bind("click", function(
										e) {
											stopBubble(e)
										});
										$("body").bind("click", function() {
													...
													result) {
														if (result.length > 0) {
															if (result[0].spStatus == 0) {
																location.replace("/products/" + result[0].fkProductId + ".html")
															} else {
																location.replace("/lotteryDetail/" + result[0].spellbuyProductId + ".html")
															}
														}
													};
												$.get("/products/getGoodsPeriodInfo.action", {
													id: _GoodsID,
													startDate: _period
												}, function(
												data) {
				...
				$.getScript(skinDomain + "/js/jquery.mousewheel.js?date=20150513", LoadPaneJs)
			}
			if (_CodeState == 2) {
				bindLotteryTimeFun()
			} else {
			...
			var loadUserInfoJs = function() {
					var LoadPageJs = function() {
							var initParamsFun = function() {
									...
									$.get("/products/checkUserLimitBuy.action", "id=" + _GoodsID, function(result) {
										_IsLogined = true;
										if (result.code == 0) {
					...
					$.getScript(skinDomain + "/js/userinfofun.js?date=20150214", LoadPageJs)
				};
			$.getScript(skinDomain + "/js/ajaxfun.js?date=20150213", loadUserInfoJs)
		};
	...
	function delCookie(name) {
		...
		if (cval != null) {
			document.cookie = name + "=" + cval + ";id=1ypg;path=/;expires=" + exp.toGMTString() + ";domain=" + $domain
		}
	}
});