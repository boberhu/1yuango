...
$(function() {
	...
	//夺宝
	var hidproductLimit = parseInt($("#hidproductLimit").val());
	if(hidproductLimit!=0){
		count=10;//限购产品可以使用10积分购买
	}
	var aD = $("#checkPoint");
	var txtUsePoint = $("#txtUsePoint");
	var spPoint = $("#spPoint");
	var ay = $("#ddBalanceBox");
	var bn = $("#checkBalance");
	var aX = $("#spBalance");
	...
	var bs = function(a) {
			...
			return '<p class="gray3">' + a + '</p><cite class="gray9">您已开启小额免密支付，≤' + a8 + ".00元无需支付密码</cite>"
		};
	var aL = function(a, d) {
			var c = "";
			switch (a) {
			...
			case "cartEmpty":
				...
				break;
			case "usePoint":
				c = '<div class="z-popUp z-pop-box">' + bs("您确定使用积分抵扣吗？") + '<a href="javascript:;" title="取消" class="z-btn-cancel">取消</a><a href="javascript:;" title="确定" class="z-btn-determine">确定</a></div>';
				var b = function() {
						aN.find("a.z-btn-determine").one("click", function() {
							$(this).prev("a").hide();
							$(this).html("正在提交，请稍候").attr("class", "z-btn-cancel").attr("title", "正在提交，请稍候");
							d();
							return false
						});
						aN.find("a.z-btn-cancel").click(function() {
				...
				break;
			case "usePB":
				c = '<div class="z-popUp z-pop-box">' + bs("您确定使用积分+余额支付吗？") + '<a href="javascript:;" title="取消" class="z-btn-cancel">取消</a><a href="javascript:;" title="确定" class="z-btn-determine">确定</a></div>';
				var b = function() {
						aN.find("a.z-btn-determine").one("click", function() {
							$(this).prev("a").hide();
							$(this).html("正在提交，请稍候").attr("class", "z-btn-cancel").attr("title", "正在提交，请稍候");
				...
				break;
			case "useBalance":
				c = '<div class="z-popUp z-pop-box">' + bs("您确定使用账户余额支付吗？") + '<a href="javascript:;" title="取消" class="z-btn-cancel">取消</a><a href="javascript:;" title="确定" class="z-btn-determine">确定</a></div>';
				var b = function() {
						aN.find("a.z-btn-determine").one("click", function() {
							$(this).prev("a").hide();
							$(this).html("正在提交，请稍候").attr("class", "z-btn-cancel").attr("title", "正在提交，请稍候");
				...
				break;
			case "pointErr":
				c = '<div class="z-popUp z-sold-out"><i class="z-arrows"></i>使用积分输入不对哦！</div>';
				$.PageDialog(c, {
					W: 210,
					H: 65,
					close: false,
				...
				break;
			case "pointEmpty":
				c = '<div class="z-popUp z-sold-out"><i class="z-arrows"></i>请输入需要抵扣的积分！</div>';
				$.PageDialog(c, {
					W: 226,
					H: 65,
					close: false,
	...
	var aH = function(b) {
			...
			if (aZ > 0) {
				if (a7) {
					var d = txtUsePoint.val();
					if (d != "") {
						if (av.test(d) && parseInt(d) <= aZ && parseInt(d) % count == 0 && parseInt(d) / count <= aV) {
							aJ = parseInt(d);
							a = parseInt(aJ / count);
							spPoint.html("-￥" + a + ".00").show();
							be = 0
						} else {
							aP = false
						}
			...
			if (bd > 0) {
				if (be == -1) {
					be = 5;
					if (aY <= 0) {
						$("#spPayTitle").html("您的账户余额不足，请选择以下方式完成支付")
					}
				} else {
					if (be == 0) {
						be = 2
	...
	if (aZ >= count) {
		...
		if (aC > 0) {
			...
			aD.parent().prev("span").html("您的积分<i>" + aZ + "</i>，使用积分");
			var aS = parseInt(aB / count);
			var aW = (aC > aS ? aS : aC) * count;
			txtUsePoint.val(aW)
		}
		aD.click(function() {
			var a = true;
			if ($(this).find("b").length > 0) {
				...
				$(this).parent().prev("span").html("使用积分支付，您的积分<i>" + aZ + "</i>");
				txtUsePoint.val("")
			} else {
				a7 = true;
				$(this).html(aT).parent().prevAll("cite").show();
				$(this).parent().prev("span").html("您的积分<i>" + aZ + "</i>，使用积分");
				var b = aV - a5;
				if (b > 0) {
					var c = parseInt(aB / count);
					c = (b > c ? c : b) * count;
					txtUsePoint.val(c);
					bg = c
				} else {
		...
		var bc = function(a) {
				spPoint.next("span").html("<em><b></b></em>" + a)
			};
		txtUsePoint.keyup(function(a) {
			var c = false;
			var d = txtUsePoint.val();
			if (d == "") {
				bg = 0
				...
				if (!av.test(d)) {
					...
					if (parseInt(d) == 0) {
						...
						var b = aV > parseInt(aZ / count) ? parseInt(aZ / count) : aV;
						if (parseInt(d) % count == 0 || parseInt(d) >= b * count) {
							if (parseInt(d) / count > b) {
								d = b * count;
								txtUsePoint.val(d);
								bc("最多可使用" + d + "积分")
							}
	...
	if (a6.length > 0) {
		...
		if (aR > 5) {
			ax.css("height", "331px");
			aq.html('<b class="z-arrows"></b>展开全部' + aR + "件商品")
		} else {
			...
			if (bp) {
				...
				ax.animate({
					...
					complete: function() {
						bp = true;
						a6.addClass("g-pay-unfold2");
						aq.html('<b class="z-arrows z-arrows2"></b>收起');
	...
	function() {
		$(this).parent().addClass("current").next("span").removeClass("current");
		bl.show().next("dl").hide()
	});
	$("#btnXYK").click(

	function() {
		$(this).parent().addClass("current").prev("span").removeClass("current");
	...
	var aQ = function(a, c, b) {
			$.getJSON("http://account.1yyg.com/JPData/API.ashx?action=" + a + "&" + c + "&fun=?", b)
		};
	var a9 = function() {
			var a = function() {
					txtUsePoint.focus()
			...
			bo.bind("click", function() {
				...
				var i = function() {
						$.ajax({
							...
							data: "integral=" + aJ + "&moneyCount=" + aV + "&hidUseBalance=" + a5 + "&bankMoney=" + bd + "&id=" + $("#ids").val(),
							success: function(l) {
								if (l.error) {
									alert(l.error);
								}
								if (l.length > 5) {
									location.replace(aK + "/mycart/shopok.html?id=" + l)
								} else {
									if (l == "1") {
										aL("cartEmpty")
									} else {
