$(function() {
	...
	var X = /\w+(@{1})$/i,
		S = ["qq.com", "sina.com", "163.com", "126.com", "139.com", "sohu.com"],
		A = "",
		C = 0;
	var P = function(a) {
	...
	var z = function() {
			try {
				if (/(msie\s|trident.*rv:)([\w.]+)/.test(navigator.userAgent.toLowerCase()) && parseInt($.browser.version) == 6) {
	...
	var N = function(b, a) {
			...
			$("#dd_error_msg").html('<i class="passport-icon transparent-png"></i>' + a).show(5, function() {
				z()
			});
			...
			if (i.state == 0) {
				...
				if (j.length > 0) {
					...
					if (h != null && h.type == 0 && h.src != "") {
						...
						if (h.url != "") {
							$("#loadingPicBlock").html('<a href="' + h.url + '" target="_blank"></a>')
						}
					}
				}
			}
...
/*QC.Login({
	...
	function() {
		...
		window.open("http://openapi.qzone.qq.com/oauth/show?which=ConfirmPage&response_type=code&client_id=" + $("#hidQQAppID").val() + "&redirect_uri=" + escape($www + "/qcback.html") + "&state=qq&scope=get_user_info&display=pc")
	});
	$("#wx_login_btn").click(
	...
	function() {
		SetCookie("qFromUrl", $("#hidLoginForward").val());
		if (/(msie\s|trident.*rv:)([\w.]+)/.test(navigator.userAgent.toLowerCase()) && parseInt($.browser.version) == 6) {
			window.open("https://open.weixin.qq.com/connect/qrconnect?appid=" + $("#hidWxAppID").val() + "&redirect_uri=" + escape($www + "/wxback.html") + "&response_type=code&scope=snsapi_login&state=wx#wechat_redirect")
		} else {
			var e = $("#wxLogin");
			var b = $("#LoginForm");
			var f = $("#a_return", e);
