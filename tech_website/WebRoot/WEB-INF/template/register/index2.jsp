...
<html>
  ...
  <body>
	...
	<div class="Service_Agreement">    
	    ...
	    <p>5、用户在成为<%=ApplicationListenerImpl.sysConfigureJson.getSaitName()%>的会员后，可按<%=ApplicationListenerImpl.sysConfigureJson.getSaitName()%>的积分规则享受积分获得。累积积分可用于积分商城中的相应积分商品兑换。积分规则连同与该规则相关的条款和条件，构成用户与<%=ApplicationListenerImpl.sysConfigureJson.getSaitName()%>之间的完整协议。接受本协议，即表明接受积分规则中的条款和条件。</p>
    ...
    <script language="javascript" type="text/javascript" src="<%=ApplicationListenerImpl.sysConfigureJson.getSkinUrl()%>/js/register.js"></script>
  </body>
</html>
