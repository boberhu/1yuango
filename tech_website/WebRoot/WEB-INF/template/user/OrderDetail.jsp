...
<html>
  <head>
    ...
    <link rel="stylesheet" type="text/css" href="<%=ApplicationListenerImpl.sysConfigureJson.getSkinUrl()%>/css/layoutcommodity.css?data=20131121" />
    <link rel="stylesheet" type="text/css" href="<%=ApplicationListenerImpl.sysConfigureJson.getSkinUrl()%>/css/dateinput.css?data=20131121" />
  </head>
...
<div class="R-content">
		...
		<div class="hddsp"><a title="返回" href="javascript:history.go(-1);">&lt;&lt; 返回</a><b class="gray">获得的商品 - 交易详情</b></div>
		<div class="steps"><ul>
		...
		<c:if test="${latestlottery.status==10}">
		<li class="operatingcur">1.获得商品<span class="steps5"></span></li>
		<li class="operatingcur">2.确认收货地址<span class="steps5"></span></li>
		<li class="operatingcur">3.商家发货<span class="steps5"></span></li>
		<li class="operatingcur">4.确认收货<span class="steps4"></span></li>
...
<c:if test="${latestlottery.status==4}">
<p>当前状态：等待晒单</p>
<input type="button" onclick="javascript:location.href='/user/PostSingleAdd-${id }.html';" value="立即晒单" class="orangebut" /><br />
<span>您可通过晒单获得相应积分奖励哦。</span>
</c:if>
...
<input type="hidden" value="${id }" id="hidOrderNO" name="hidOrderNO" />
     <table border="0" cellspacing="0" cellpadding="0">
             <tbody>
           		<tr>
                  <td width="140">${fn:substring(latestlottery.announcedTime,0,19) }</td>
                     <td width="400">恭喜您<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>成功，请尽快确认收货地址，以便我们为您配送！</td>
                     <td>操作：<%=ApplicationListenerImpl.sysConfigureJson.getSaitName()%></td>
                 </tr>
	                    ...
	                    <div class="form">
	                        <ul>
	                            ...
	                            <li>
	                                ...
	                                <span>
	                                    <input type="text" style="width: 300px;" class="text" maxlength="100" size="100" id="shipAddress" name="shipAddress" />&nbsp;该项中不需要填写省、市、区信息。
	                            ...
	                            <li>
	                                <label>邮政编码：</label>
	                                <span>
	                                    <input type="text" class="text" id="shipZip" maxlength="6" size="20" name="shipZip" />&nbsp;<a href="http://alexa.ip138.com/post/Search.aspx" class="blue" target="_blank">邮编查询</a>
	                            ...
	                            <li>
	                                ...
	                                <span>
	                                    <input type="text" class="text" maxlength="20" size="20" id="shipName" name="shipName" />&nbsp;请填写全名，避免"赵先生"、"孙生"、"李小姐"等，以免被误签
   ...
   <div class="information">
        ...
        <table border="0" cellspacing="0" cellpadding="0">
         ...
         <tr class="infoTitle">
             ...
             <tr>
              <td class="commodity">
              <a target="_blank" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/products/${latestlottery.spellbuyProductId }.html">
              <img width="70" height="70" border="0" align="left" alt="${latestlottery.productTitle }" src="<%=ApplicationListenerImpl.sysConfigureJson.getImgUrl()%>${latestlottery.productImg }" /></a>
              <em class="orange">(第${latestlottery.productPeriod }期)</em><a target="_blank" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/products/${latestlottery.spellbuyProductId }.html">${latestlottery.productName }</a>
    ...
    <script language="javascript" type="text/javascript" src="<%=ApplicationListenerImpl.sysConfigureJson.getSkinUrl()%>/js/orderdetail.js"></script>
