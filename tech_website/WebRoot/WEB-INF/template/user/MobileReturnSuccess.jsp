...
<html>
  <head>
    ...
    <script type="text/javascript">
        $(function(){var A=10;var B=function(){if(A==1){location.replace("/user/index.html")}else{$("#spanNum").html(A);A--}};setInterval(B,1000)})
    </script>
  </head>
...
<script type="text/javascript">
	...
	<div class="R-content">
		<div class="member-t"><h2>手机验证</h2></div>
		<div class="tel_verification_ok">
                <h4>验证成功</h4>
                <ul>
                    <li><span>验证成功！</span></li>
                    <li>您现在可以使用您的手机号 <span>${user.phone}</span> 或邮箱账号 <span>
                    <c:choose>
                    	<c:when test="${user.mailCheck=='3'}">
                    		还未验证
                    	</c:when>
                    	<c:otherwise>
                    		${user.mail }
                    ...
                    <li>您还可以：<a href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>">开始<%=ApplicationListenerImpl.sysConfigureJson.getShortName()%></a> <a href="/user/index.html">去个人中心首页</a></li>
                    <li>如没有任何操作，系统将在 <span id="spanNum">10</span> 秒后自动跳转到个人中心首页！</li>
                </ul>
            </div>
