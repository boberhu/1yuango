...
<div class="R-content">
	...
	<div class="member-t"><h2>佣金明细</h2></div>
      <div class="total">
  	    <dt>
      	    </dt><dd>累计收入：<b class="orange">${user.commissionCount }</b> 元</dd>
      	    <dd>累计(提现/充值)：<b class="orange">${user.commissionMention }</b> 元</dd>       
      	    <dd>佣金余额：<b class="orange">${user.commissionBalance}</b> 元</dd>
      	    <dd><a class="bluebut" title="申请提现" href="/user/ApplyMention.html">申请提现</a><a class="orangebut" title="充值到<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>账户" href="/user/ApplyMention.html?t=">充值到<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>账户</a></dd>
          <dd class="gray02">佣金余额可实时充值到您的<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>帐户，满100元时才可申请提现。</dd>
	  </div>
   <div class="record-tit">
    ...
    <script language="javascript" type="text/javascript" src="<%=ApplicationListenerImpl.sysConfigureJson.getSkinUrl()%>/js/commissionquery.js?data=20131121"></script>
