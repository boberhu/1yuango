...
<script type="text/javascript">
    ...
    $("div.sidebar-nav").find("h3").each(function(i,v){
		...
		<div class="R-content">
			<div class="subMenu">
                <a href="/user/UserRecharge.html" title="网银充值" >网银充值</a>
                <a href="/user/userCardRecharge.html" class="current" title="充值卡充值">充值卡充值</a>
            </div>
            <div class="member-t"><h2>账户充值</h2></div>
            <div align="center" style="padding-top: 50px;padding-left: 5px;padding-bottom:150px;">
           	<span style="color: #f60;font-size: 24px;margin-bottom: 10px;float: left;">请输入卡密号码：</span>
            <input type="text" value="" id="txtCard" maxlength="12" style="width: 380px;border: 2px solid #f60;color: #333;padding: 0 3px;height: 34px;line-height: 34px;font-size: 24px;float: left;"/>
            <input type="submit" title="确定充值" value="确定充值" style="background: none repeat scroll 0 0 #f60;border: 1px solid #f70;color: #fff;padding: 6px 25px;font-size: 14px;" class="bluebut imm" id="cardButton" />
            </div>
            
            <div class="member-t">
			<h2 style="float: none;">一元<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>充值卡问答：</h2>
			<br />
			  <p style="font-size: 14px; font-weight: bold;color: #f60;padding: 5px;">1.什么是一元<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>充值卡？</p>
			  <p style="padding: 5px;">答：一元<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>卡是一元<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>(<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>)发行的自有预付卡，卡内资金可以在一元<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>商城内使用。</p>
			  <p style=" font-size: 14px; font-weight: bold;color: #f60;padding: 5px;">2.如何购买一元<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>卡？</p>
			  <p style="padding: 5px;">答：为了保障您的合法权益，请通过一元<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>认可的正规网点购买一元<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>卡，或淘宝进行购买。</p>
			  <p style="font-size: 14px; font-weight: bold;color: #f60;padding: 5px;">3.购买一元<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>卡是否有手续费？</p>
			  <p style="padding: 5px;">答：无需手续费，所有一元<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>充值卡都是全额到账！</p>
			</div>
        </div>
    ...
    <script language="javascript" type="text/javascript">
    		...
    		$.ajax({
			   ...
			   success: function(msg){
			   		if(msg=="yes"){
			   			...
			   			$.PageDialog("<dl class=\"sAltFail\"><dd>充值失败!该卡密不存在或已经被冲值，请重试!</dd></dl>",{W:360, H:50,autoClose:false});
			   			setTimeout(function (){window.location.href = $www+"/user/userCardRecharge.html";}, 3000);
			   		}
			   },
			   error:function(){
			   		alert("网络错误，请稍后再试！");
    	...
    	$("#cardButton").click(function(){
    		var key = $("#txtCard").val();
    		if(key==""){
    			$.PageDialog("<dl class=\"sAltFail\"><dd>请输入卡密号码!</dd></dl>",{W:180, H:50,autoClose:true});
    			$("#txtCard").focus();
    			return false;
    		}else if(key.length<12){
    			$.PageDialog("<dl class=\"sAltFail\"><dd>输入卡密号码长度不正确!</dd></dl>",{W:250, H:50,autoClose:true});
    			$("#txtCard").focus();
    			return false;
    		}else{
