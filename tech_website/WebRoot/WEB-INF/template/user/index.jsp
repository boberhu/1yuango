...
<html>
  <head>
    ...
    <link rel="stylesheet" type="text/css" href="<%=ApplicationListenerImpl.sysConfigureJson.getSkinUrl()%>/css/layouthome.css?data=20131121" />
  </head>
...
<p class="sid-line"></p>
<h2 class="sid-cur"><a href="index.html"><b></b>我的<%=ApplicationListenerImpl.sysConfigureJson.getSaitName()%></a></h2><p class="sid-line"></p>
    ...
    $("div.sidebar-nav").find("h3").each(function(i,v){
        ...
        <div class="center">
		    <div class="per-info">
			    <ul>
			        ...
			        <br />
			        <span><a class="blue" target="_blank" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/u/${user.userId }.html"><s></s> <%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/u/${user.userId }.html</a></span></li>
				    ...
				    <li class="account-money"><em class="gray02">帐户余额：</em><span class="money-red"><s></s>${user.balance}</span>&nbsp;&nbsp;<a class="blue" title="充值" href="/user/UserRecharge.html">充值</a></li>
			    </ul>
			    <input type="hidden" id="hidUID" value="${user.userId }"/>
	    		<input name="hidRefTitle" type="hidden" id="hidRefTitle" />
	    		<c:if test="${user.mobileCheck==3}">
	    		<div class="tips orange">
                      <em></em>为了及时收到您获得的<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>商品信息，请立即验证您的手机号码！<a class="blue" href="/user/MobileChecking.html">立即验证</a><a class="close" id="ColseMessage"></a>
                 </div>
	    		</c:if>
			        <div class="per-invitation gray02" id="divInvited">
				        <label>复制链接邀请好友：</label><span><input type="text" onpaste="return false" value="" id="txtInfo" /></span> <a class="bluebut" href="javascript:;" id="btnCopy">复制</a>
			        </div>
		    ...
		    <s:iterator value="productList" var="buyHistoryJSONs">
		         <div class="scroll-list">
			        <div class="buy-com"><p class="buy-pic"><a href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/products/${buyHistoryJSONs.productId }.html" target="_blank">
			        <img border="0" alt="" src="<%=ApplicationListenerImpl.sysConfigureJson.getImgUrl()%>${buyHistoryJSONs.headImage }" /></a></p>
			        <div class="buy-rcon"><p class="buy-name"><a class="blue" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/products/${buyHistoryJSONs.productId }.html" target="_blank">(第${buyHistoryJSONs.productPeriod }期)${buyHistoryJSONs.productName } </a></p>
			        ...
			        <div class="Progress-bar">
						...
						<ul class="Pro-bar-li">
							...
							<li class="P-bar03"><em>${buyHistoryJSONs.productPrice - buyHistoryJSONs.currentBuyCount}</em>剩余人次</li>
						</ul>
					</div>
			        <p><a class="orangebut" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/products/${buyHistoryJSONs.productId }.html" target="_blank">马上夺宝</a></p></div></div>
		        </div>
	        </s:iterator>
	    ...
	    <script type="text/javascript" language="javascript">
			...
			var FormatHtml = function (str){
				...
				.replace(/\[s:(\d+)\]/ig,'&lt;img src="http://skin.591jx.net/Images/$1.gif" alt="" /&gt;')
		    ...
		    <div class="gg-content">
			    ...
			    <ul class="gg-list">
			    	<s:iterator value="newsList" var="news">
			            <li><span class="point"></span><span class="info"><a title="${news.title }" class="gray" target="_blank" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/news/${news.newsId }.html">${news.title }</a></span></li>
			    	</s:iterator>
			    </ul>
		    </div>
	    </div>
    </div>
    <script language="javascript" type="text/javascript" src="<%=ApplicationListenerImpl.sysConfigureJson.getSkinUrl()%>/js/member.js?data=20131121"></script>
