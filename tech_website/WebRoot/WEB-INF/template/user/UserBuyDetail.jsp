...
<html>
  <head>
    ...
    <title><%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>详情_<%=ApplicationListenerImpl.sysConfigureJson.getSaitName()%></title>
    <link rel="stylesheet" type="text/css" href="<%=ApplicationListenerImpl.sysConfigureJson.getSkinUrl()%>/css/layoutframe.css" />
    <link rel="stylesheet" type="text/css" href="<%=ApplicationListenerImpl.sysConfigureJson.getSkinUrl()%>/css/comm.css" />
    <link rel="stylesheet" type="text/css" href="<%=ApplicationListenerImpl.sysConfigureJson.getSkinUrl()%>/css/layoutrecords.css" />
  </head>
...
<div class="left">
	...
	<div class="head-but">
		<a class="blue" href="/user/UserPhoto.html">修改头像</a><a class="blue fr" href="/user/MemberModify.html">编辑资料</a>
	</div>
...
<ul><li class="sid-cur"><a href="/user/UserBuyList.html"><%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>记录</a></li>
...
<li class=""><a href="/user/UserRecharge.html">账户充值</a></li></ul>
...
<script type="text/javascript">
        ...
        <div class="yg_record_goods">
        	<c:choose>
               	<c:when test="${buyHistoryJSON.buyStatus==1}">
               		<a title="${buyHistoryJSON.productName }" class="fl-img" target="_blank" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/lotteryDetail/${buyHistoryJSON.spellbuyProductId }.html"><img src="<%=ApplicationListenerImpl.sysConfigureJson.getImgUrl()%>${buyHistoryJSON.productImg }" /></a>
               		<div class="yg_record_r">
               		<li><span class="orange">(第${buyHistoryJSON.productPeriod }期)</span> <a class="blue" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/lotteryDetail/${buyHistoryJSON.spellbuyProductId }.html" target="_blank">${buyHistoryJSON.productName }</a></li>
	                <li class="gray02">本期商品<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>已结束 【获得者：<a class="blue" target="_blank" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/u/${buyHistoryJSON.winUserId }.html">${buyHistoryJSON.winUser }</a> 幸运编号：${buyHistoryJSON.winId } 揭晓时间：${buyHistoryJSON.winDate }】</li>
                	<li><a class="bluebut" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/lotteryDetail/${buyHistoryJSON.spellbuyProductId }.html" target="_blank">查看详情</a></li>
                	</div>
               	</c:when>
               	 <c:otherwise>
               	 	<a title="${buyHistoryJSON.productName }" class="fl-img" target="_blank" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/products/${buyHistoryJSON.spellbuyProductId }.html"><img src="<%=ApplicationListenerImpl.sysConfigureJson.getImgUrl()%>${buyHistoryJSON.productImg }" /></a>
               	 	<div class="yg_record_r">
               	 	<li><span class="orange">(第${buyHistoryJSON.productPeriod }期)</span> <a class="blue" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/products/${buyHistoryJSON.spellbuyProductId }.html" target="_blank">${buyHistoryJSON.productName }</a></li>
                	<li><a class="bluebut" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/products/${buyHistoryJSON.spellbuyProductId }.html" target="_blank">查看详情</a></li>
                	</div>
                </c:otherwise>
               </c:choose>
        </div>
        <input type="hidden" value="${resultCount }" id="resultCount"/>
        <input type="hidden" value="${buyHistoryJSON.spellbuyProductId }" id="productId"/>
        <input type="hidden" value="${userId }" id="userId"/>
        <div class="goods-tit gray02">
            <p class="fl">本期商品您总共<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %><span>${buyHistoryJSON.buyCount }</span>人次 拥有<span>${buyHistoryJSON.buyCount }</span>个<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>码</p><a class="blue fr" href="UserBuyList.html">&lt;&lt; 返回列表</a></div>
    ...
    <script type="text/javascript" src="<%=ApplicationListenerImpl.sysConfigureJson.getSkinUrl()%>/js/userbuydetail.js"></script>
</body>
</html>
