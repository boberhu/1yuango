...
<!--			</div>-->
				...
				<ul id="ulGoodsList" class="item">
					<s:iterator value="productList" var="Products">
						<li codeid="${Products.productId }" class="goods-iten">
							<div class="pic">
								<a title="${Products.productTitle }" target="_blank" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/productInfo/${Products.productId }.html" rel="nofollow">
								...
								<img  class="scrollLoading" data-url="<%=ApplicationListenerImpl.sysConfigureJson.getImgUrl()%>${Products.headImage }" src="<%=ApplicationListenerImpl.sysConfigureJson.getImgUrl()%>/Images/pixel.gif" title="${Products.productTitle }" alt="${Products.productName }" />
								</a>
								<p style="display: none;" name="buyCount"></p>
							</div>
							<p class="name">
								<a title="${Products.productTitle }" target="_blank" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/productInfo/${Products.productId }.html">${Products.productName }</a>
							</p>
							<p style="color: #999;padding: 0 10px 0;text-align: left;">价值：<span style="background-position: 0 -3px;background-repeat: no-repeat;color: #f60;padding-left: 10px;">￥${Products.productPrice}.00</span>
							</p>
			...
			<div style="display: none;" class="goods_loding" id="divLoadingLine">
				<img border="0" src="<%=ApplicationListenerImpl.sysConfigureJson.getImgUrl()%>/Images/goods_loading.gif" alt="" style="vertical-align: middle;" />
					正在努力加载中......
			</div>
...
<!--			</div>-->
		...
		<!-- end -->
		<script type="text/javascript" src="<%=ApplicationListenerImpl.sysConfigureJson.getSkinUrl()%>/js/productlist.js"></script>
