...
<html>
	   	...
	   	<div class="wait_list hidden" id="divResult" style="display: block;">
	   		<c:choose>
	   			<c:when test="${fn:length(successCartList)>0}">
				        ...
				        <h2 class="orange Fb">恭喜您，支付成功！请等待系统为您揭晓！</h2>
				     ...
				     <div class="wait_listCon">
				        ...
				        <s:iterator value="successCartList" var="successCart">
					        <li class="wait_Lodity"><span class="wait_Cw01">${successCart.buyDate}</span><span class="wait_Cw02"><a class="blue" title="(第${successCart.productPeriod }期)${successCart.productName }" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/products/${successCart.productId }.html">(第${successCart.productPeriod }期)${successCart.productName }</a></span><span class="wait_Cw03">${successCart.buyCount}</span><span class="wait_Cw04"><a href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/user/UserBuyDetail-${successCart.productId }.html" target="_blank">查看详情</a></span></li>
				        </s:iterator>
				        </ul>
		           </div>
	   			</c:when>
	   			<c:otherwise>
	   				<div class="wait_list_tips ">
				        ...
				        <h2 class="orange Fb">哎呀！<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>失败了。已经被人抢拍了吧。</h2>
				        <p>您现在可以 <a class="blue" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/mycart/index.html">查看购物车</a> 或 <a class="blue" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>">继续购物</a></p>
				        </dd></dl>
				    </div>
				    <div class="wait_listCon">
    ...
    <script language="javascript" type="text/javascript" src="<%=ApplicationListenerImpl.sysConfigureJson.getSkinUrl()%>/js/successcartlist.js"></script>
