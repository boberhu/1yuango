...
<html>
	<head>
		...
		<title>最新<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>记录 <%=ApplicationListenerImpl.sysConfigureJson.getSaitTitle() %></title>
...
<!--				<a href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/getAllBuyRecord.html">查看历史<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>记录&gt;&gt;</a>-->
			...
			<div id="recordList">
			<s:iterator value="newRecordList" var="newRecords" status="i">
				<c:choose>
					<c:when test="${i.index%2==0}">
						<ul class="Record_content" rel="${newRecords.productStyle }">
							<li class="time">${newRecords.buyDate }</li>
							<li class="nem"><a href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/u/${newRecords.userId }.html" target="_blank" class="blue">${newRecords.buyer }</a></li>
							<li class="name"><a href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/products/${newRecords.productId }.html" title="${newRecords.productTitle }">（第${newRecords.productPeriod }期）${newRecords.productName }</a></li>
...
<!--				<a rel="nofollow" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/getAllBuyRecord.html">查看历史<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>记录&gt;&gt;</a>-->
		...
		<script type="text/javascript" src="<%=ApplicationListenerImpl.sysConfigureJson.getSkinUrl()%>/js/newrecord.js"></script>
	</body>
</html>
