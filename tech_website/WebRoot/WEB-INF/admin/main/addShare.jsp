...
<html>
  ...
  <body>
 	...
 	<div style="float: left; width: 100%;">
      ...
      <div class="col-sm-10">
         <input type="text" placeholder="请输入标题" class="form-control" name="shareinfo.shareTitle" value="${shareinfo.shareTitle }" />
       </div>
      <label class="col-sm-2 control-label" for="inputEmail3">日期</label>
      <div class="col-sm-10">
      <c:choose>
 				<c:when test="${announcedTime!=null}">
		 			<input type="text" placeholder="请输入日期" style="width: 180px;float: left;" class="form-control" name="shareinfo.shareDate" value="${fn:substring(announcedTime,0,19) }" />
 				</c:when>
 				<c:otherwise>
		 			<input type="text" placeholder="请输入日期" style="width: 180px;float: left;"  class="form-control" name="shareinfo.shareDate" value="${fn:substring(shareinfo.shareDate,0,19) }" />
 				</c:otherwise>
		</c:choose>
       </div>
      </div>
    </div>
		 <p style="float: left; padding: 50px;"><textarea id="editor" name="shareinfo.shareContent" cols="60" rows="10">${shareinfo.shareContent }</textarea></p>
	</form>
 	</div>
	...
	<div align="center"  style="float: right; padding: 20px;">
		<s:iterator value="shareimageList" var="shareimages">
			<img name="_small" id="${shareimages.images  }" alt="<%=url.getString("img")%>/UserPost/Big/${shareimages.images }" src="<%=url.getString("img")%>/UserPost/Small/${shareimages.images }"/>
		</s:iterator>
	</div>
