...
<html>
 ...
 <form action="/admin_back/doUpdateRole.action" method="post">
 <input type="hidden" name="role.id" value="${role.id!}"/>
 <table width="100%" cellspacing="0" cellpadding="0" style="">
		<tbody style="">
		<tr>
			<td align="left" width="150px;"><font color="red">*</font>角色名称：</td>
			<td align="left"><input type="text" id="name" name="role.name" value="${role.name!}" class="input-text wid100 bg" />
            </td>
		</tr>
        <tr>
			<td align="left" width="150px;"><font color="red">*</font>角色描述：</td>
			<td align="left">
			<textarea class="input-text" style="height:50px; width:400px" id="description" name="role.description">${role.description!}</textarea>
			</td>
		</tr>
		<tr>
			...
			<td align="left">
				<#if funcRoot??>
				<@p.tree root=funcRoot showDeep=1 isUrl=false isCheckBox=true checkBoxName="functions.id" valueContent=stack.findValue("role.functions.{id}") vld="{required:true}"/>
				</#if>
			</td>
		</tr>
        <tr>
