    ...
    throws IOException
  {
    ...
    try
    {
      ...
      try
      {
        ...
        {
          textBytes = getTextEntry((String)textEntry.getKey(), (String)textEntry.getValue(), 
            charset);
          out.write(entryBoundaryBytes);
          out.write(textBytes);
        }
        Set<Map.Entry<String, FileItem>> fileEntrySet = fileParams.entrySet();
        for (Object fileEntry : fileEntrySet)
        {
          FileItem fileItem = (FileItem)((Map.Entry)fileEntry).getValue();
          byte[] fileBytes = getFileEntry((String)((Map.Entry)fileEntry).getKey(), fileItem.getFileName(), 
