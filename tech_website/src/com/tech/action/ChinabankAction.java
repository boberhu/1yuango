...
import com.tech.tenpay.util.TenpayUtil;
...
public class ChinabankAction extends BaseAction
{
    ...
    throws ServletException, IOException
  {
    ...
    String v_mid = ApplicationListenerImpl.sysConfigureJson.getChinabankPartner();
    String v_url = ApplicationListenerImpl.sysConfigureJson.getWwwUrl() + "/chinabank/returnUrl.action";
    
    String key = ApplicationListenerImpl.sysConfigureJson.getChinabankKey();
    String remark1 = integral;
    String remark2 = "[url:=" + ApplicationListenerImpl.sysConfigureJson.getWwwUrl() + "/chinabank/notifyUrl.action]";
    String v_oid = out_trade_no;
    String v_amount = String.valueOf(moneyCount);
    String v_moneytype = "CNY";
    String text = v_amount + v_moneytype + v_oid + v_mid + v_url + key;
    String v_md5info = MD5Util.encode(text);
    String pmode_id = payBank;
    

    String body = "<form action=\"https://pay3.chinabank.com.cn/PayGate\" method=\"POST\" name=\"E_FORM\">";
    body = body + "<input type=\"hidden\" name=\"v_md5info\"    value=" + v_md5info + " size=\"100\">";
    body = body + "<input type=\"hidden\" name=\"v_mid\"   value=" + v_mid + ">";
    body = body + "<input type=\"hidden\" name=\"v_oid\"   value=" + v_oid + ">";
    body = body + "<input type=\"hidden\" name=\"v_amount\"   value=" + v_amount + ">";
    body = body + "<input type=\"hidden\" name=\"v_moneytype\"  value=" + v_moneytype + ">";
    body = body + "<input type=\"hidden\" name=\"v_url\"   value=" + v_url + ">";
    body = body + "<input type=\"hidden\"  name=\"remark1\" value=" + remark1 + ">";
    body = body + "<input type=\"hidden\"  name=\"remark2\" value=" + remark2 + ">";
    body = body + "<input type=\"hidden\"  name=\"pmode_id\" value=" + pmode_id + "></form>";
    body = body + "<script>document.forms['E_FORM'].submit();</script>";
    ...
    if (productCartList.size() > 0)
    {
      ...
      try
      {
        ...
        else if (userPayType.intValue() == 2)
        {
          ...
          {
            flag = false;
            Struts2Utils.render("text/html", "<script>alert(\"积分不足，支付失败！\");window.location.href=\"/mycart/index.html\";</script>", new String[] { "encoding:UTF-8" });
          }
        }
            ...
            if (count.intValue() > 0)
            {
              try
              {
                if (userPayType.intValue() == 1)
                {
                  ...
                  commissionpoints.setDetailed(ApplicationListenerImpl.sysConfigureJson.getShortName() + "商品编码(" + spellbuyproduct.getSpellbuyProductId() + ")支付" + count + "元获得积分");
                ...
                else if (userPayType.intValue() == 2)
                {
                  ...
                  Commissionpoints commissionpoints = new Commissionpoints();
                  commissionpoints.setDate(DateUtil.DateTimeToStr(new Date()));
                  commissionpoints.setDetailed(ApplicationListenerImpl.sysConfigureJson.getShortName() + "商品编码(" + spellbuyproduct.getSpellbuyProductId() + ")积分抵扣");
